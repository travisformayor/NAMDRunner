...
This document defines the design for **Job Chains** and how they integrate with **NAMD Configuration Templates** to enable multi-stage molecular dynamics workflows in NAMDRunner.

> **Note**: For complete template system design (field types, validation, dynamic forms, etc.), see [Templates.md](Templates.md). This document focuses on job chaining mechanisms and how templates enable multi-stage workflows.
...
> **For complete template system design**, see [Templates.md](Templates.md)
>
> This section covers only the template aspects relevant to job chaining.

### 4.1 Template Types from Tutorial
...
### 4.2 How Templates Enable Job Chaining

**Key Insight**: Multi-stage workflows = **job chains** (each stage continues from previous outputs), not multiple template types.
...
- `template_values` (user's filled form values)
- Input files (previous job's restart files become current job's inputs)

**What Stays Constant**:
- `template_id` (same simulation type throughout chain)
- Original input files (`.pdb`, `.psf`, `.prm` - read-only to NAMD)

See [Templates.md](Templates.md) for full template system architecture, field types, validation rules, and implementation details.
...
> **See [Templates.md](Templates.md) Section 7** for complete template data structures, field types, and validation rules.

**Job Chain Integration**: Jobs store `template_id` (string) to reference which template was used, plus `template_values` (HashMap) containing user's filled form values.
...
> **For complete template definitions and implementation details**, see [Templates.md](Templates.md)

This section covers only template aspects specific to job chaining and restart mechanisms.

### 9.1 Restart Configuration Blocks
...
> **See [Templates.md](Templates.md) Section 4.7** for complete custom validator design and implementation details.
...
> **See [Templates.md](Templates.md) Section 8** for complete UI component architecture, including:
> - Template store pattern (following ClusterConfig pattern)
> - DynamicField component (field type rendering)
> - DynamicConfigForm component (form section rendering)
> - Integration into CreateJobPage

**Job Continuation UX**: Dynamic forms pre-populate with parent job's template values, allowing users to modify parameters for next stage (e.g., change restraint file, adjust simulation length).
