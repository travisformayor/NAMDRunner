...
This document provides a complete reference of NAMD configuration patterns, command execution, and file management proven to work with the CURC Alpine cluster and molecular dynamics workflows.
...
#### Restart Template After Timeout
```tcl
# Template for restarting a simulation that hit walltime limit
...
### Required Module Sequence (Alpine Cluster)
Always load modules before any NAMD execution:
...
```bash
# Modern MPI execution with CPU affinity for optimal performance
...
- Use `namd3` not `namd2` (Alpine uses NAMD 3.0.1)
- Requires MPI execution with `mpirun` for parallel performance
- CPU affinity settings (`+setcpuaffinity +pemap`) improve performance
